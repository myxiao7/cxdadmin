...
import com.zh.cxdadmin.entity.PushEntity;
...
//            System.out.println("收到了自定义消息。消息内容是：" + bundle.getString(JPushInterface.EXTRA_EXTRA));
            ...
            if (!TextUtils.isEmpty(result)) {
                Type type = new TypeToken<PushEntity>() {
                }.getType();
                ...
                switch (entity.getMessage_type()) {
                    ...
                    case "6":
                        //新的订单
                        MyNotificationManager.getInstance().showNotifi(context, "初小丁", "有新的订单");
                        Intent intent1 = new Intent(BaseApplication.ORDERWAIT);
                        context.sendBroadcast(intent1);
                        break;

                    case "5":
                        //新的审核信息
                        MyNotificationManager.getInstance().showNotifi(context, "初小丁", "有新的加盟商");
                        Intent intent2 = new Intent(BaseApplication.VERIFYWAIT);
